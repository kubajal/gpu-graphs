...
Logger *get_static_logger(void) { return &static_logger; }

static void vlog_impl(FILE *out, const char *prefix, const char *fmt, va_list ap) {
    fputs(prefix, out);
    vfprintf(out, fmt, ap);
    fputc('\n', out);
}
...
void set_log_level(Logger *logger, enum LOG_LEVEL log_level) { logger->log_level = log_level; }
